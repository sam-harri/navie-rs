...
impl JsonOutputManager {
    ...
    pub fn collect_timestep(
        ...
        if grid.dimensions.0 != self.nx || grid.dimensions.1 != self.ny {
            return Err(io::Error::new(
                ...
                format!(
                    "Grid dimensions ({},{}) do not match JsonOutputManager dimensions ({},{})",
    ...
    pub fn write_final_output(
        ...
        if self.collected_data.is_empty() {
            info!(
                "No data collected, skipping JSON output to {}.",
                self.output_filepath
            );
            return Ok(());
        }
        info!(
            "Writing collected data to JSON file: {}...",
            self.output_filepath
        );
...
mod tests {
    ...
    fn test_should_collect_logic() {
        let manager_freq10 = JsonOutputManager {
            ...
            output_frequency: Some(10),
            collected_data: vec![],
            nx: 10,
            ny: 10,
        };
        assert!(manager_freq10.should_collect(0, 100, false));
        assert!(!manager_freq10.should_collect(1, 100, false));
        assert!(manager_freq10.should_collect(10, 100, false));
        assert!(manager_freq10.should_collect(99, 100, true));
        assert!(manager_freq10.should_collect(100, 100, true));
        let manager_freq0 = JsonOutputManager {
            output_filepath: "".to_string(),
            output_frequency: Some(0),
            collected_data: vec![],
            nx: 10,
            ny: 10,
        };
        assert!(manager_freq0.should_collect(0, 100, false));
        assert!(!manager_freq0.should_collect(1, 100, false));
        assert!(manager_freq0.should_collect(100, 100, true));
    ...
    fn test_collect_and_write() -> io::Result<()> {
        ...
        assert_relative_eq!(p1_flat[0].as_f64().unwrap(), 5.0);
        dir.close()?;
        Ok(())
    }
}
